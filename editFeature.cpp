...
editFeature::editFeature (QWidget *pt, OPTIONS *op, MISC *mi, int32_t num):
  QDialog (pt, (Qt::WindowFlags) Qt::WA_DeleteOnClose)
{
  ...
  strcpy (image_name, "");
  mod_feature_type = options->default_feature_type;
  ...
  if (new_feature)
    {
      ...
      bfd_record.feature_type = options->default_feature_type;
    }
  else
    {
  ...
  QGroupBox *idBox = new QGroupBox (tr ("Record number and feature type"), this);
  ...
  QGroupBox *typeBox = new QGroupBox (tr ("Feature type"), this);
  QHBoxLayout *typeBoxLayout = new QHBoxLayout;
  typeBox->setLayout (typeBoxLayout);
  featureType = new QComboBox (typeBox);
  featureType->setToolTip (tr ("Change the feature type for this feature (and set the default feature type)"));
  featureType->setWhatsThis (featureTypeText);
  featureType->setEditable (false);
  featureType->addItem (tr ("Hydrographic"));
  featureType->addItem (tr ("Informational"));
  connect (featureType, SIGNAL (currentIndexChanged (int)), this, SLOT (slotFeatureTypeChanged (int)));
  typeBoxLayout->addWidget (featureType);
  idBoxLayout->addWidget (typeBox);

  if (new_feature)
    {
      featureType->setCurrentIndex (options->default_feature_type);
    }
  else
    {
    }
  ...
  QLabel *depthLabel = new QLabel (depthBox);
  depthLabel->setText (depth);
  depthLabel->setAlignment (Qt::AlignHCenter);
  depthLabel->setToolTip (tr ("Feature Depth"));
  depthBoxLayout->addWidget (depthLabel);
  dimBoxLayout->addWidget (depthBox);
...
void 
editFeature::slotFeatureTypeChanged (int index)
{
  mod_feature_type = bfd_record.feature_type = index;
}
...
editFeature::slotSnippetViewerReadyReadStandardError ()
{
  ...
  emit readStandardErrorSignal (imageProc);
}
...
editFeature::slotAssignSnippet ()
{
  ...
  fd->selectNameFilter (tr ("Image (*.bmp *.jpg *.png *.gif *.xpm)"));
  ...
  if (fd->exec () == QDialog::Accepted)
    {
      ...
      if (file.isEmpty())
        ...
        {
          strcpy (image_name, file.toLatin1 ());
...
{
  strcpy (image_name, filename.toLatin1 ());
...
editFeature::slotApplyEditFeature ()
{
  ...
  options->default_feature_type = mod_feature_type;
  ...
  strcpy (bfd_record.remarks, options->last_feature_remarks.toLatin1 ());

  options->last_feature_description = description->text ();

  strcpy (bfd_record.description, options->last_feature_description.toLatin1 ());
      ...
      if (!misc->bfd_open)
        {
          ...
          char feature_dir[512];
          strcpy (feature_dir, name.toLatin1 ());
          sprintf (misc->abe_share->open_args[0].target_path, "%s/%s.bfd", feature_dir, gen_basename (misc->abe_share->open_args[0].list_path));
          ...
          fd.setNameFilter (tr ("Binary Feature Data file (*.bfd)"));
                  ...
                  if ((misc->bfd_handle = binaryFeatureData_create_file (misc->abe_share->open_args[0].target_path, misc->bfd_header)) < 0)
                    {
                      ...
                      QMessageBox::warning (parent, "pfmEdit3D", tr ("Unable to create feature file\nReason: ") + msg);
                      return;
                    }
  ...
  //  Try to set or unset the PFM_SELECTED_FEATURE/PFM_DESIGNATED_SOUNDING flag in the data (depending on the confidence and/or the feature type).
...
editFeature::slotLidar ()
{
  ...
  for (int32_t i = 0 ; i < LIDAR_TYPES ; i++) lidarBox->addItem (lidarType[i]);
