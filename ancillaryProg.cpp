...
ancillaryProg::ancillaryProg (QWidget *parent, nvMapGL *ma, OPTIONS *op, MISC *mi, int32_t prog, int32_t lock_point):
  QProcess (parent)
{
  ...
  int32_t ret = buildCommand (progString, actionString, misc, lock_point, &cmd, &args, options->kill_switch[prog]);
      ...
      switch (misc->abe_share->modcode)
        ...
        case PFM_SHOALS_1K_DATA:
          if (misc->abe_share->modified_point >= 0)
            {
              int32_t set_point = misc->abe_share->modified_point;
